...
Drupal.behaviors.slashcomments = function(context) {
  ...
  $("form[@id ^= 'slashcomments-moderation-form']").submit(function() {
    ...
    var moderated = function (data) {
        ...
        $(form_obj).find('div.sdot_loader').empty();

        $(form_obj).find('div.sdot_form').slideToggle(300, function(){
            $(form_obj).find('div.sdot_form_status').html(data.status).slideDown(300);
        });

        $('#comment-'+cid).find('.sdot_rating').fadeOut("slow", function() {
    ...
    var error_handler = function(xhr, ajaxOptions, thrownError) {
        ...
        $(form_obj).find('div.sdot_loader').empty();
        $(form_obj).find('div.sdot_form_status').html(Drupal.settings.slashdot.error_status).css('display','inline');
    ...
    $(this).find('div.sdot_form_status').css('display','none');
    $(this).find('div.sdot_loader').html('<img src="'+Drupal.settings.slashdot.load_image_path+'"/>');
